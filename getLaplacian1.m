function [L]=getLaplacian1(I,epsilon,win_size)
  ...
  L=sparse(row_inds,col_inds,vals,img_size,img_size); % Create the laplacian matrix, holding the sum of second part of equation 12. Note that the row,colomn pairs may have repeated position, so all elements of vals that have duplicate row and column'indices are added together, see the help.
  
  %sumA=sum(L,2);
  %L=spdiags(sumA(:),0,img_size,img_size)-L; % Sum of ach row of Laplacian matrix is zero.
